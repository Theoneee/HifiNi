...
public class DouYinLoadingView extends View {
    ...
    public DouYinLoadingView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        String color=typedArray.getString(R.styleable.DouYinLoadingView_progressColor);
        mDefaultWidth = (int) typedArray.getDimension(R.styleable.DouYinLoadingView_minWidth, 600);
        mDefaultHeight = (int) typedArray.getDimension(R.styleable.DouYinLoadingView_minHeight, 5);
        mMinProgressWidth = (int) typedArray.getDimension(R.styleable.DouYinLoadingView_minProgressWidth, 20);
        mSpeed = (int) typedArray.getDimension(R.styleable.DouYinLoadingView_speed, 50);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mProgressWidth<mWidth){
            //注意执行此步骤是mProgressWidth值有可能大于view宽度
            mProgressWidth+=mSpeed;
        }else{
